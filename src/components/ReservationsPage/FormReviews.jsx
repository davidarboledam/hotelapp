...
const FormReviews = ({ formIsOpen, bookSelected, setFormIsOpen, setBookSelected }) => {
	...
	return (
		<div className={`review__container flex-container ${formIsOpen || 'review__closed'}`}>
			<section className="review">
				<i onClick={handleExit} className="bx bx-x review__close"></i>
				<h2 className="review__title">Rate the service</h2>
				<article className="review__reserve">
					<header className="review__reserve-header">
						<img className="review__reserve-img" src={bookSelected?.hotel.images[0].url} alt={bookSelected?.hotel.name} />
					</header>
					<div className="review__reserve-body">
						<h3 className="review__reserve-title">{bookSelected?.hotel.name}</h3>
				...
				<form onSubmit={handleSubmit(submit)} className="review__form grid-container">
					...
					<label className="review__form-field grid-container">
						...
						<textarea {...register('comment')} className="review__form-textarea"></textarea>
					</label>
